...
st.markdown(
    "Ce modèle utilise **EfficientNetB0** (Transfer Learning) pour identifier 10 types d'objets avec ~93% de précision."
)
...
try:
    ...
    st.error(
        f"Erreur : Impossible de charger le modèle. Vérifiez que le fichier se trouve bien dans 'models/best_model_efficientnet_aug.h5'.\n\nDétail : {e}"
    )

# 4. Interface Utilisateur
st.write("---")
uploaded_file = st.file_uploader(
    "Glissez une image ici (Chat, Avion, etc.)", type=["jpg", "png", "jpeg"]
)
